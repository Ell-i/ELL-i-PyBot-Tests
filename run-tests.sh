...
run_test() {
    pybot  --outputdir $PATH_TO_TESTS/test-results/$1 $PATH_TO_TESTS/test-suites/$1.rest
}


case "$1" in
  list)
        ls -1 test-suites/|grep ".rest"|awk -F '.' '{print $1}'
        ;;
  help)
	...
	echo "          help           This help document"
        ;;
  run-all)
	TESTS=`ls -1 test-suites/|grep ".rest"|awk -F '.' '{print $1}'`
